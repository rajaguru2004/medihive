...
| 38 | **Nothing could open a submitted intake.** `submit()` rendered the case, wrote it whole and attached it to the patient record, and `CaseTakingController`'s header ended its argument with "a clinician reads a finished intake through the submission on the patient record" — a route that did not exist. Every case-taking route is under `PatientSelfGuard` and refuses a staff caller by design | The feature's whole output was write-only. A patient could spend twenty minutes answering questions and no doctor had any way to read the answers | ✅ `CaseSubmissionsController` — `GET /case-taking/submissions[?patientId]` and `/:submissionId`, gated on `CASE_TAKING_READ` with no write anywhere on it. A second controller rather than two more routes, because the guard is mounted per class. The app reads it from an **Intake** tab on the patient hub, into `CaseIntakeView` |
| 39 | **The fixture world's doctor was missing `CASE_TAKING_READ` and `PATIENT_DOCUMENT_READ`**, which the seed has granted since P9. `world_roles.dart` says of that table: "Copied from the backend seed. Nothing derives these — they *are* the source" | Drift in the direction that hides a feature: the hub's new Intake tab rendered the refusal screen for the one role it was written for, and the flow failed on `/no-access`. Any future staff-facing case-taking screen would have failed the same way, and the fixture would have looked like the authority | ✅ both added to the doctor and the nurse, matching the seed exactly |
| 40 | A stored `RenderedItem` carries `presence` as a token and **no `presenceText`** — `toReviewView` adds it for the patient's screen, and the submission writer does not | A clinician's screen mapping six tokens to six sentences itself is the blank-where-"Patient unsure"-belongs bug, one layer along | ✅ `withPresenceLabels` fills it from the stored token on the way out — a label, not a re-render. The screen then prints it only when it differs from `display`, or a line reads "None reported" twice |
| 41 | **`patient-scoped-routes.spec.ts` matched a controller that merely *named* the guard.** It collected any source containing the string `PatientSelfGuard`, and a comment on `appointments.controller.ts` explaining why that controller scopes its own writes instead was enough to pull it in | The spec then reported four offending `:id` params on a **staff** controller the guard is not mounted on — where nothing rewrites them. A false positive on a safety net is how the net gets loosened: the obvious fix is to relax the rule | ✅ the detector keys on `PatientSelfGuard` inside a `@UseGuards(...)`, or on `@PatientScope(` — mounted, not mentioned. Still covers the three genuinely guarded controllers, and confirmed non-vacuous by reintroducing a bare `:id` on `case-taking.controller.ts` |
